...
* Summer 2018
* Project 4
* Description: receives requests from client to decrypt text
************************************/
...
void decrypt(char textIn[70000], char textOut[70000], char key[70000])
{
	int i;
	int decrypted;
	int textNum;
	...
	{
		// convert characters to numbers for decryption
		textNum = charToNum(textIn[i]);
		keyNum = charToNum(key[i]);

		// decrypt
		decrypted = (textNum - keyNum + 27) % 27;

		// add decrypted character to the outgoing text
		textOut[i] = numToChar(decrypted);
	}
}
...
int main(int argc, char *argv[])
{
	...
	// listens for requests to a maximum of 5
	listen(socketFD, 5);
	...
	while (1)
	{
		...
		// checks if spawnpid is the parent process
		if (spawnpid > 0)
		{
			continue;
		}

		// make sure that request is not from otp_enc
		readStuff(clientSocket, buffer, &endChar);
		if (strcmp(buffer, "otp_enc") == 0)
		{
			// send back error message
			sprintf(message, "ERROR otp_enc cannot use otp_dec_d\n");
