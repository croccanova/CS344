...
* Summer 2018
* Project 4
* Description: sends requests to server to encrypt text
************************************/
...
int main(int argc, char *argv[])
{
	...
	// tell server that otp_enc is sending the request
	strcpy(buffer, "otp_enc");
	writeStuff(socketFD, buffer, &endChar);

	// checks if the client and server dont match
	if (strncmp(buffer, "ERROR otp_dec cannot use otp_enc_d", 35) == 0)
	{
		// send error to stderr
